...
impl Plugin for InputPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<PlayerInputEvent>().add_systems(
            Update,
            input_from_keyboard.run_if(in_state(state::App::Game(state::Game::Input))),
        );
    }
}
...
pub fn input_from_keyboard(
    ...
    mut next_state: ResMut<NextState<state::App>>,
) {
    if keyboard.just_pressed(KeyCode::ArrowLeft) {
        input_evw.send(PlayerInputEvent::Left);
        next_state.set(state::App::Game(state::Game::Calculate));
