...
use super::super::constant::layout;
...
impl Position {
    ...
    pub fn to_transform(&self, z: f32) -> Transform {
        let x = (-(layout::GRID_WIDTH as f32) / 2.0 + self.x as f32 + 0.5)
            * (layout::TILE.width + layout::TILE.margin.unwrap_or_default().horizontal);
        let y = (-(layout::GRID_HEIGHT as f32) / 2.0 + self.y as f32 + 0.5)
            * (layout::TILE.height + layout::TILE.margin.unwrap_or_default().vertical);
